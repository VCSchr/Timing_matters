...
library(plyr) # data wrangling, version 1.8.9
library(dplyr) # data wrangling, version 1.1.4
library(vegan) # create RDA plots, version 2.6-8
library(lme4) # create LMM, version 1.1-35.5
library(emmeans) # conduct pairwise comparisons of LMM between treatments of one time point, version 1.10.5
library(DAAG) # check QQ plots, version 1.25.6
library(Rmisc) # calculate CI, version 1.5.1
library(ggplot2) # create plots, version 3.4.3
library(BiodiversityR) # conduct pairwise comparisons of the RDA analysis, version 2.16-1
library(indicspecies) # For IndVal analysis, version 1.7.15
library(ggpubr) # arrange plots, version 0.6.0
library(betapart) # for tunover and nestedness calculations, version 1.6
```
...
For the transplant treatment, the temperature was calculated as weighted means based on the exposure temperatures during the deployments at the respective sites.
...
# keep a full dataframe for the calculation of the decompositon rate k
leaf_weight_k <- leaf_weight
...
library(parameters)
skewness(leaf_weight$DLM) # 2.268 
```

data not normally distributed. identify outliers (strong tail visible)
...
plot(DLM_lmer) # point cloud looDLMs evenly distributed --> normal distribution
qreference(residuals(DLM_lmer)) # normality not perfect, but oDLM

drop1(DLM_lmer, test = 'Chisq')
# interaction is highly significant (p = 0.002285)
# checDLM next at which time points is there a difference between treatments and where not
anova(DLM_lmer)
```
...
## decompostion rate k
```{r}
leaf_weight_k$k <- (-log((leaf_weight_k$afdm_end / leaf_weight_k$afdm_start)) )/
                   (leaf_weight_k$mean_temp * leaf_weight_k$number.days)
```

### Statistical analysis k
investigate data 
```{r}
hist(leaf_weight_k$k) 

qqnorm(leaf_weight_k$k, datax=TRUE)
qqline(leaf_weight_k$k, datax=TRUE)

library(parameters)
skewness(leaf_weight_k$k) # 2.268 
```

data not normally distributed. But due to the logarithmic character of the variable a transformation is not possible and the non-normality is not driven by single outliers, but by a right tail. This tail suggests that a linear model is still best way to go and other distributions are not better suited. 

##### model with decomposition rate k
```{r}
# create the second random factor stream + treatment
leaf_weight_k$fac_repli <- paste(leaf_weight_k$stream, leaf_weight_k$treatment)

k_lmer <- lmer(k ~ time.point + treatment + time.point:treatment + (1|stream) + (1|fac_repli), data = leaf_weight_k)

plot(k_lmer) # point cloud looks evenly distributed --> normal distribution
qreference(residuals(k_lmer)) # normality not perfect, but ok

drop1(k_lmer, test = 'Chisq')
# interaction is highly significant (p = 0.001353 **)
...
conduct pairwise comparison
```{r}
k_inter <- lsmeans(k_lmer, ~ treatment | time.point )
k_inter_dunn1 <- contrast(k_inter, 'pairwise')
(k_inter_dunn_sum1 <- summary(k_inter_dunn1, infer = TRUE, adjust = 'mvt'))
```

##### create Figure S4
create plot decomposition rates k
```{r}
k_CI <- group.CI(k ~ time.point + treatment,
                   data = leaf_weight_k,
                   ci = 0.95)

# change order of the factor time point
k_CI$time.point <- factor(k_CI$time.point,
                             levels = c("April", "June", "August", "September"))

# rename transplant treatments in order to enable fit to the plot
k_CI$treatment <- gsub("transplant", "T", k_CI$treatment)
k_CI$treatment <- gsub("forest", "F", k_CI$treatment)
k_CI$treatment <- gsub("viti", "V", k_CI$treatment)

# for indication of significance
ann_text_1 <- data.frame(treatment = "T", k.mean = 0.003, lab = "\U25AA", 
                       time.point = factor("April",levels = c("April", "June", "August", "September")))  
# Difference forest - viti April
ann_text_2 <- data.frame(treatment = "V", k.mean = 0.002885, lab = "\U25AA", 
                       time.point = factor("April",levels = c("April", "June", "August", "September"))) 
# Difference transplant - viti April
# ann_text_3 <- data.frame(treatment = "T", k.mean = 0.20, lab = "\U25AA",  # unicode hex for square
#                        time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# # Difference forest - viti June
ann_text_4 <- data.frame(treatment = "V", k.mean = 0.00288, lab = "**", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference transplant - viti June
# ann_text_5 <- data.frame(treatment = "F", k.mean = 0.195, lab = "\U25AA", 
#                        time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# # Difference forest - transplant August
ann_text_6 <- data.frame(treatment = "T", k.mean = 0.00299, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti August
ann_text_7 <- data.frame(treatment = "V", k.mean = 0.00288, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference transplant - viti August
# ann_text_8 <- data.frame(treatment = "F", k.mean = 0.1945, lab = "*", 
#                        time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# # Difference forest - transplant September
ann_text_9 <- data.frame(treatment = "T", k.mean = 0.00299, lab = "**", 
                       time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti September
# ann_text_10 <- data.frame(treatment = "V", k.mean = 0.195, lab = "\U25AA",
#                        time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# # Difference transplant - viti September
ann_text <- rbind(ann_text_1, ann_text_2, ann_text_4,  ann_text_6, ann_text_7,  ann_text_9)


# for lines below significance indications
fv <- data.frame(a = c(1.0, 2, 2, 3), b = rep(c(0.00298), 4)) 
mv <- data.frame(a = c(2.1, 2.5, 3, 3.3), b = rep(c(0.00285), 4))
nv <- data.frame(a = c(0.7, 1, 1, 1.9), b = rep(c(0.0029), 4))

p1_k <-  ggplot(k_CI, aes(x = treatment, y = k.mean)) +
  geom_pointrange(aes(y = k.mean, ymin = k.lower, ymax = k.upper))+
  geom_point(mapping = aes(y = k.mean), size = 3) +
  facet_wrap(~time.point, nrow = 1) +
  ylab('decompositon rate k per dday')+
  xlab("")+
  ylim(0.00125, 0.003)+
...
svg(filename="Figure_S4_decomposition_rate.svg", width=10, height=7.5, pointsize=12)
p1_k
dev.off()
...
# rename transplant treatments in order to enable fit to the plot
DLM_CI$treatment <- gsub("transplant", "T", DLM_CI$treatment)
DLM_CI$treatment <- gsub("forest", "F", DLM_CI$treatment)
DLM_CI$treatment <- gsub("viti", "V", DLM_CI$treatment)

# for indication of significance
ann_text_1 <- data.frame(treatment = "T", DLM.mean = 0.199, lab = "**",
                       time.point = factor("April",levels = c("April", "June", "August", "September")))
# Difference forest - viti April
ann_text_2 <- data.frame(treatment = "V", DLM.mean = 0.193, lab = "**",
                       time.point = factor("April",levels = c("April", "June", "August", "September")))
# Difference transplant - viti April
ann_text_3 <- data.frame(treatment = "T", DLM.mean = 0.20, lab = "\U25AA",  # unicode hex for square
                       time.point = factor("June",levels = c("April", "June", "August", "September")))
# Difference forest - viti June
ann_text_4 <- data.frame(treatment = "V", DLM.mean = 0.193, lab = "**",
                       time.point = factor("June",levels = c("April", "June", "August", "September")))
# Difference transplant - viti June
ann_text_5 <- data.frame(treatment = "F", DLM.mean = 0.195, lab = "\U25AA",
                       time.point = factor("August",levels = c("April", "June", "August", "September")))
# Difference forest - transplant August
ann_text_6 <- data.frame(treatment = "T", DLM.mean = 0.199, lab = "***",
                       time.point = factor("August",levels = c("April", "June", "August", "September")))
# Difference forest - viti August
ann_text_7 <- data.frame(treatment = "V", DLM.mean = 0.193, lab = "***",
                       time.point = factor("August",levels = c("April", "June", "August", "September")))
# Difference transplant - viti August
ann_text_8 <- data.frame(treatment = "F", DLM.mean = 0.1945, lab = "*",
                       time.point = factor("September",levels = c("April", "June", "August", "September")))
# Difference forest - transplant September
ann_text_9 <- data.frame(treatment = "T", DLM.mean = 0.199, lab = "***",
                       time.point = factor("September",levels = c("April", "June", "August", "September")))
# Difference forest - viti September
ann_text_10 <- data.frame(treatment = "V", DLM.mean = 0.195, lab = "\U25AA",
                       time.point = factor("September",levels = c("April", "June", "August", "September")))
# Difference transplant - viti September
...
# rename transplant treatments in order to enable fit to the plot
taxa_CI$treatment <- gsub("transplant", "T", taxa_CI$treatment)
taxa_CI$treatment <- gsub("forest", "F", taxa_CI$treatment)
taxa_CI$treatment <- gsub("viti", "V", taxa_CI$treatment)

# create significance indication
taxa_text_1 <- data.frame(treatment = "T", taxa_rich.mean = 22, lab = "**", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti August
taxa_text_2 <- data.frame(treatment = "V", taxa_rich.mean = 20.6, lab = "**", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti August
taxa_text_3 <- data.frame(treatment = "T", taxa_rich.mean = 22, lab = "*", 
                       time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti September
taxa_text_4 <- data.frame(treatment = "V", taxa_rich.mean = 21, lab = "\U25AA", # unicode hex for square!
                       time.point = factor("September",levels = c("April", "June", "August", "September"))) 
...
### sporulation rate
calculate total sporulation rate based on 
```{r}
conidia_corr$conidia_total <- rowSums(conidia_corr[4:60])
```


Statistics to sporulation rate
```{r}
spor_lmer1 <- lmer(conidia_total ~ time.point + treatment + time.point:treatment + (1|stream), data = conidia_corr)

drop1(spor_lmer1, test = 'Chisq')
# interaction not significant, reduce model complexity

spor_lmer2 <- lmer(conidia_total ~ time.point + treatment + (1|stream), data = conidia_corr)

drop1(spor_lmer2, test = 'Chisq') # timepoint significant
...
```{r}
spor_inter <- lsmeans(spor_lmer2, ~ treatment | time.point )

spor_inter_dunn1 <- contrast(spor_inter, 'pairwise')
(spor_inter_dunn_sum1 <- summary(spor_inter_dunn1, infer = TRUE, adjust = 'mvt'))
```
...
```{r}
spor_inter2 <- lsmeans(spor_lmer2, ~ time.point)

spor_inter_dunn2 <- contrast(spor_inter2, 'pairwise')
(spor_inter_dunn_sum2 <- summary(spor_inter_dunn2, infer = TRUE, adjust = 'mvt'))
```
April significantly different from other time points, Tendency of difference between June and September, due to seasonality? Since August and September are similar, the different deployment time is likely not the main reason for differences in sporulation rate


#### create Figure S3
create plot of decomposition data
```{r}
spor_CI <- group.CI(conidia_total ~ time.point + treatment,
                   data = conidia_corr,
                   ci = 0.95)

# change order of factor
spor_CI$time.point <- factor(spor_CI$time.point,
                             levels = c("April", "June", "August", "September"))

# rename transplant treatments in order to enable fit to the plot
spor_CI$treatment <- gsub("transplant", "T", spor_CI$treatment)
spor_CI$treatment <- gsub("forest", "F", spor_CI$treatment)
spor_CI$treatment <- gsub("viti", "V", spor_CI$treatment)

p1_spor <-  ggplot(spor_CI, aes(x = treatment, y = conidia_total.mean)) +
  geom_pointrange(data = spor_CI, aes(y = conidia_total.mean, ymin = conidia_total.lower, ymax = conidia_total.upper))+
  geom_point(data = spor_CI, mapping = aes(y = conidia_total.mean), size = 3) +
  facet_wrap(~time.point, nrow = 1) +
  ylab('sporulation rate')+
  xlab("")+
  theme_bw()+
  theme(axis.text.x = element_text(size = 18, color="black"), axis.ticks.x = element_blank(), 
         axis.text.y = element_text(colour="black", size = 18), 
...
png(filename="Figure_S3_sporulation_rate.png", width=10, height=7.5, pointsize=12, units = "in", res = 800)
p1_spor
dev.off()
```
...
##### Create figures for single time points, Figure S7
...
svg(filename="Figure_S7_a_RDA_conidia_April.svg", width=8, height=8, pointsize=12)
p1_RDA_conid <- plot(rda_conid_5, 
          scaling = 'symmetric', 
          type = 'n',
          xlab = 
...
svg(filename="Figure_S7_b_RDA_conidia_June.svg", width=8, height=8, pointsize=12)
p1_RDA_conid <- plot(rda_conid_5, 
          scaling = 'symmetric', 
          type = 'n',
          xlab = 
...
svg(filename="Figure_S7_c_RDA_conidia_August.svg", width=8, height=8, pointsize=12)
p1_RDA_conid <- plot(rda_conid_5, 
          scaling = 'symmetric', 
          type = 'n',
          xlab = 
...
svg(filename="Figure_S7_d_RDA_conidia_September.svg", width=8, height=8, pointsize=12)
p1_RDA_conid <- plot(rda_conid_5, 
          scaling = 'symmetric', 
          type = 'n',
          xlab = 
...
##### create Figure S8
Plot similar to Figure 2 without ploygons
```{r}
scl = 'symmetric'
# arrange size of single plot
si_sc <- scores(rda_conid_5, choices = 1:2, scaling = scl, display = 'sites')
...
png(filename="Figure_S8_RDA_conidia_wo_polygon.png", width=15, height=15, pointsize=10, units = "cm", res = 800)
p1_RDA_conid <- plot(rda_conid_5, 
          scaling = 'symmetric', 
          type = 'n',
          xlab = 
...
# rename transplant treatments in order to enable fit to the plot
turn_CI$treatment <- gsub("transplant", "T", turn_CI$treatment)
turn_CI$treatment <- gsub("viti", "V", turn_CI$treatment)
turn_CI$treatment <- gsub("forest", "F", turn_CI$treatment)


# create significance indication
turn_text_1 <- data.frame(treatment = "T", value.mean = 0.45, lab = "***", 
                       time.point = factor("April",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti April
turn_text_2 <- data.frame(treatment = "F", value.mean = 0.42, lab = "**", 
                       time.point = factor("April",levels = c("April", "June", "August", "September"))) 
# Difference forest - transplant April
turn_text_3 <- data.frame(treatment = "T", value.mean = 0.45, lab = "***", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti June
turn_text_4 <- data.frame(treatment = "F", value.mean = 0.42, lab = "**", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference forest - transplant June
turn_text_5 <- data.frame(treatment = "V", value.mean = 0.43, lab = "**", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti June
turn_text_6 <- data.frame(treatment = "T", value.mean = 0.45, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti August
turn_text_7 <- data.frame(treatment = "V", value.mean = 0.43, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti August
...
# rename transplant treatments in order to enable fit to the plot
nest_CI$treatment <- gsub("transplant", "T", nest_CI$treatment)
nest_CI$treatment <- gsub("forest", "F", nest_CI$treatment)
nest_CI$treatment <- gsub("viti", "V", nest_CI$treatment)


# create significance indication
nest_text_1 <- data.frame(treatment = "F", value.mean = 0.44, lab = "***", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference forest - transplant June
nest_text_2 <- data.frame(treatment = "V", value.mean = 0.45, lab = "***", 
                       time.point = factor("June",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti June
nest_text_3 <- data.frame(treatment = "T", value.mean = 0.475, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti August
nest_text_4 <- data.frame(treatment = "V", value.mean = 0.45, lab = "***", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti August
nest_text_5 <- data.frame(treatment = "T", value.mean = 0.475, lab = "***", 
                       time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# Difference forest - viti September
nest_text_6 <- data.frame(treatment = "V", value.mean = 0.45, lab = "***", 
                       time.point = factor("September",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti September
...
save plots
```{r}
fig_S5_beta_diss <-ggarrange(p1_turn, p1_nest, nrow = 2,labels = c("(a)","(b)"))

svg(filename="Figure_S5_beta_diss.svg", width=10, height=15, pointsize=12)
fig_S5_beta_diss
dev.off()
```
...
png(filename="Figure_S9_c_RDA_DNA_all_fungi.png", width=15, height=15, pointsize=10, units = "cm", res = 800)
...
# rename transplant treatments in order to enable fit to the plot
ASV_CI$treatment <- gsub("transplant", "T", ASV_CI$treatment)
ASV_CI$treatment <- gsub("viti", "V", ASV_CI$treatment)
ASV_CI$treatment <- gsub("forest", "F", ASV_CI$treatment)

# add one tendency to plot
taxa_text <- data.frame(treatment = "V", ASV_rich.mean = 85, lab = "\U25AA", 
                       time.point = factor("August",levels = c("April", "June", "August", "September"))) 
# Difference trans. - viti August
...
png(filename="Figure_S9_b_RDA_DNA_Hyph_ASVs.png", width=15, height=15, pointsize=10, units = "cm", res = 800)
...
# rename transplant treatments in order to enable fit to the plot
ASV_hyph_CI$treatment <- gsub("transplant", "T", ASV_hyph_CI$treatment)
ASV_hyph_CI$treatment <- gsub("viti", "V", ASV_hyph_CI$treatment)
ASV_hyph_CI$treatment <- gsub("forest", "F", ASV_hyph_CI$treatment)
...
png(filename="Figure_S9_a_RDA_DNA_hyph.png", width=15, height=15, pointsize=10, units = "cm", res = 800)
...
# rename transplant treatments in order to enable fit to the plot
tax_DNA_CI$treatment <- gsub("transplant", "T", tax_DNA_CI$treatment)
tax_DNA_CI$treatment <- gsub("forest", "F", tax_DNA_CI$treatment)
tax_DNA_CI$treatment <- gsub("viti", "V", tax_DNA_CI$treatment)
...
```{r}
fig_S6_richness <-ggarrange(p1_taxa, p1_tax_DNA, p1_ASV_hyph, p1_ASV, nrow = 2, ncol= 2, labels = c("(a)","(b)", "(c)", "(d)"))


svg(filename="Figure_S6_richness.svg", width=16.5, height=15, pointsize=11)
fig_S6_richness
dev.off()
